...
**Dentro:** schema novo (`tenants`, `tenant_members`, `tenant_invites`, `api_keys`), `tenant_id` em `projects`, soft-delete, helpers server (`requireAuth`, `requireTenantRole`, `requireTenantAccess`), helpers SQL (`has_tenant_role`, `is_super_admin`), RPCs de ownership, RLS refinada, rotas novas, UI de gestão. **`engines.project_id` vira `NOT NULL`** após migration — todo engine precisa de um project; engines sem project ficam fora da chain `engine → project → tenant` necessária para a RLS.
...
`create_tenant` aceita apenas `name` obrigatório; `owner_id` e `billing_id` são opcionais e default para o super admin criador. O owner definitivo é designado depois via `transfer_primary_ownership` + `set_billing_owner`. Form de UI só pede `name`.
...
| `ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_RENAME_TENANT` | `rename_tenant` |
...
- Nenhuma rota `/api/*` (exceto `/api/auth/*`) responde 200 sem auth. Rotas `/api/schema` e `/api/schemas/*` exigem `requireAuth` (são metadados da API, mas seguem o mesmo critério).
