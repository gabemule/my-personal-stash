...
export async function POST(request: NextRequest, { params }) {
  ...
  // Carrega user uma vez — usado no sendInviteEmail abaixo.
  ...
  if (newRole === "owner") {
    ...
    if (!auth.isSuperAdmin && !isPrimaryOwner) {
  ...
  if (error) {
    // 23505 = unique_violation (tenant_invites_pending_unique). Indica race
    // com outro POST concorrente que inseriu convite pendente pro mesmo
    // (tenant, email) entre o lookup acima e o insert. Mapeia pro mesmo 409
    // que o caminho síncrono.
    if ((error as { code?: string }).code === "23505") {
      return NextResponse.json({ error: "PENDING_INVITE_EXISTS" }, { status: 409 })
    }
...
```ts
// DELETE — manager+ revoga convite pendente (user-only)
