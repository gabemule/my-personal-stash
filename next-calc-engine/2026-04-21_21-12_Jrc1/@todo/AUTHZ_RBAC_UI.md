...
| `stores/tenantStore.ts` | Criar | **Thin store** — só `currentTenantId` + `tenants` (hidratado do `SessionContext` do server). `selectTenant` chama `POST /api/session/current-tenant` → `router.refresh()` → limpa `useWorkspaceStore`. `role`/`isSuperAdmin` NÃO vivem aqui (ver `TenantSummary.isPrimaryOwner` / `isBillingOwner` / `isSuperAdminView` + menu filtrado pelo server) |
...
| `components/PermissionGate/index.tsx` | Criar | Wrapper que deriva visibilidade do **menu filtrado pelo server** (não lê `role` bruto). Uso: `<Gate menuId="settings/api-keys">` |
| `app/tenants/[id]/settings/page.tsx` | Criar | Página "Configurações" (edit nome) |
| `app/tenants/[id]/settings/members/page.tsx` | Criar | Lista membros + actions |
| `app/tenants/[id]/settings/api-keys/page.tsx` | Criar | (vindo da Fase 2) Listar/gerar/revogar keys |
...
export default async function AuthedLayout({ children }: { children: React.ReactNode }) {
  ...
  // (SessionContext NÃO expõe isSuperAdmin; derivamos via menu: se há item
  // "admin/tenants/new" no menu, é super admin.)
  const isSuperAdminView = session.menu.some((m) => m.id === "admin/tenants/new")

  if (session.tenants.length === 0) {
    redirect(isSuperAdminView ? "/admin/tenants/new" : "/no-tenant")
  }
...
**Thin store.** Fonte primária dos dados é o `SessionContext` do server
(injetado via `SessionHydrator`). `role`/`isSuperAdmin` **não** vivem aqui —
UI que precise de gate usa o menu (ver §5) ou `TenantSummary.isPrimaryOwner`/
`isBillingOwner`/`isSuperAdminView`.
...
interface TenantState {
  ...
  hydrate: (tenants: TenantSummary[], currentTenantId: string | null) => void
...
export const useTenantStore = create<TenantState>((set, get) => ({
  ...
  hydrate: (tenants, currentTenantId) => set({ tenants, currentTenantId }),
  currentTenant: () =>
...
export async function POST(request: NextRequest) {
  ...
  // RPC create_tenant faz tudo atômico: insert em tenants + tenant_members.
  const { data: tenantId, error } = await auth.supabase.rpc("create_tenant", {
    _name: name,
    _owner_id: ownerId,
...
export async function PATCH(request: NextRequest, { params }) {
  ...
  const { error } = await auth.supabase.rpc("change_member_role", {
...
export async function DELETE(request: NextRequest, { params }) {
  ...
  if (isPrimaryOwnerTarget) {
    ...
    const { error } = await auth.supabase.rpc("force_remove_primary_owner", {
      _tenant_id: params.id,
  ...
  // Caso normal
  const { error } = await auth.supabase.rpc("remove_member", {
    _tenant_id: params.id,
...
//
// Implementar como RPC Postgres para atomicidade:
//   select transfer_primary_ownership(_tenant_id, _new_owner_id)
// que faz em transação:
//   1. checa invariantes (newPrimaryOwnerId existe e é owner)
//   2. update tenants set owner_id = new_owner
```
...
//
// RPC Postgres recomendado:
//   select set_billing_owner(_tenant_id, _user_id)
// em transação: check + update tenants set billing_id = _user_id.
```
...
O gate deriva visibilidade do **menu filtrado no server** (Fase 1 §12) — não
do `role` bruto, que nem é exposto ao client. Isso mantém a decisão de
"quem vê o quê" num único lugar (`lib/menu.ts`).
...
interface Props {
  /** id do item de menu que habilita este conteúdo (ex: "settings/api-keys"). */
  menuId: string
  children: React.ReactNode
  fallback?: React.ReactNode
}

/** Renderiza children apenas se o menu do user contém o item `menuId`. */
export function PermissionGate({ menuId, children, fallback = null }: Props) {
...
Uso:
```tsx
<PermissionGate menuId="builder">
  <button onClick={createProject}>Novo Projeto</button>
</PermissionGate>
```

> O `sessionStore` é um companion thin do `tenantStore` com `{ user, menu }`
> hidratado pelo mesmo `<SessionHydrator>` do layout. Se preferir, dá pra
> fundir os dois — a doc mantém separados pra deixar claro quem é dado
> de tenant (`tenantStore`) vs dado de sessão (`sessionStore`).
...
### `app/tenants/[id]/settings/page.tsx`
Página de configurações do tenant: editar nome (owner+), ver info, botão "Soft-delete tenant" (owner, com confirmação dupla).

### `app/tenants/[id]/settings/members/page.tsx`
...
### `app/tenants/[id]/settings/api-keys/page.tsx`
