...
- [ ] Declarar `export const runtime = "nodejs"` explicitamente em `app/api/calc/[...segments]/route.ts`. Cache em `Map` module-scope **não funciona em edge runtime** (cada invocação é um isolate novo). Ver `RUNTIME_REFACTOR_PLAN.md` seção 1.2.d.
- [ ] Criar `lib/server/compiledCache.ts` como **casa canônica** do cache (compartilhada entre `/api/calc` consumidor e endpoints de invalidação em A.2):
  ```ts
  // lib/server/compiledCache.ts
  ...
  ```
- [ ] Handler do endpoint `/api/calc/:id` passa a: (a) `getCompiled(id, () => loadEngine(id))`, (b) `run(compiled, inputs, opts)`, (c) serializar `ExecuteResult` no mesmo shape atual.
- [ ] Manter log de cache hit/miss em dev para validação (remover ou condicionar a `NODE_ENV === "development"`).

**Por quê:** hoje cada request paga 2–10ms de validação Zod + construções. Com cache, só a primeira request por processo paga; demais servem em <1ms. Manter o cache em `lib/server/` (não em `lib/runtime/`) preserva a fronteira da lib — `compiledCache` é infra do app, não do runtime; quando o runtime for extraído como pacote independente, o cache fica no serviço HTTP consumidor, não viaja junto.
...
Os 4 endpoints de `/api/engines` têm política distinta — invalidar só onde o JSON do engine efetivamente muda:

| Endpoint | Invalida? | Racional |
|---|---|---|
| `PATCH /api/engines/:id` | **Sim** (sempre que succeede) | Cobre mudança de `engine`, `name` e `project_id`. Barato: um `.delete()` numa `Map` per-request de PATCH. |
| `DELETE /api/engines/:id` | **Sim** | Engine deixou de existir; entrada no cache é órfã. |
| `POST /api/engines/:id/activate` | **NÃO** | Só muta `is_active`. O JSON de `engine` não muda, e `/api/calc/:id` não filtra por `is_active`. Invalidar seria desperdício + cache miss extra na próxima request. |
| `POST /api/engines` (create) | **NÃO** | Engine novo nunca esteve no cache. |

- [ ] Importar `invalidateCompiled` de `lib/server/compiledCache.ts` (definido em A.1) nos handlers de `PATCH` e `DELETE` de `app/api/engines/[id]/route.ts`.
- [ ] Chamar `invalidateCompiled(engineId)` **após** o `.update()` / `.delete()` do Supabase retornar sucesso (ordem importa — não invalidar antes de confirmar persistência).
- [ ] Não tocar nos handlers de `POST /api/engines` (create) nem `POST /api/engines/:id/activate`.
- [ ] Documentar no topo de `compiledCache.ts` que a invalidação é **explícita** (sem TTL, sem versionamento), e listar os dois endpoints que disparam.
- [ ] Considerar invalidação em lote futura (ex.: admin deleta categoria com N engines) — API interna se demanda aparecer. **Não implementar agora.**

**Por quê:** sem invalidação explícita, admin salva um engine novo e o próximo request ainda usa a versão compilada antiga. Cache stale em motor de cálculo fiscal = bug crítico. Listar os 4 endpoints explicitamente evita que alguém adicione um PATCH secundário no futuro sem invalidar (ex.: endpoint de "clonar engine"); a tabela do corpo vira o contrato.

**Arquivos afetados:** `app/api/engines/[id]/route.ts` (handlers `PATCH` e `DELETE`), `lib/server/compiledCache.ts` (comentário explicando a política).

**Risco:** médio. Esquecer de invalidar em algum ponto = bug silencioso. Mitigação: a tabela do corpo é a fonte única; qualquer endpoint futuro que mute o JSON do engine deve ser adicionado à tabela + invalidar.

**Como verificar:** teste manual:
- `PATCH /api/engines/:id` com mudança visível (ex.: trocar valor de constante) → próximo `POST /api/calc/:id` reflete a mudança.
- `POST /api/engines/:id/activate` → `POST /api/calc/:id` responde com o mesmo compile anterior (log de cache hit mantido).
...
> Depende de: **runtime 1.4** (classe `CalcError` formal). Entrega em duas versões: **versão ponte (Fase A1)** funciona sem a classe formal; **versão final (Fase A2)** adota `instanceof CalcError` quando a classe aterrissa.

O handler vive em `lib/server/handleCalcError.ts` e é consumido por `app/api/calc/[...segments]/route.ts` (e qualquer outro handler que chame `compile()`/`run()`).

#### Versão ponte (Fase A1 — implementada antes do runtime 1.4)

`CalcError` formal é Fase 2 do runtime. Até lá, `compile()`/`run()` lançam `Error` com `message` prefixado `"[CODE] ..."` (ver PLAN seções 1.4, 1.5 e 3.2). O handler encadeia três ramos, **nesta ordem**:

```ts
export function handleCalcError(err: unknown): NextResponse {
  // Ramo 1: ZodError cru do compile() — preserva response legado 1-pra-1
  if (err instanceof ZodError) {
    return NextResponse.json(
      {
        error: "Invalid engine config",
        validationErrors: err.issues.map(i => ({ path: i.path, message: i.message })),
      },
      { status: 422 },
    )
  }

  // Ramo 2: erro com prefix "[CODE] ..." da ponte Fase 1
  if (err instanceof Error) {
    const match = /^\[([A-Z_]+)\]\s*(.*)$/.exec(err.message)
    if (match) {
      const [, code, message] = match
      const status = code === "RESOURCE_LIMIT_EXCEEDED" ? 413 : 422
      return NextResponse.json({ error: { code, message } }, { status })
    }
  }

  // Ramo 3: fallback defensivo
...
- [ ] Criar `lib/server/handleCalcError.ts` com a estrutura acima.
- [ ] **Ramo 1 (ZodError)** preserva o shape legado `{ error: "Invalid engine config", validationErrors: [...] }` exato do `execute()` facade — zero regressão para bruno/flows e consumidores externos que já tratam `validationErrors`. Status 422.
- [ ] **Ramo 2 (prefix `[CODE]`)** reconhece os códigos emitidos pela Fase 1 do runtime: `CYCLE_DETECTED` → 422, `RESOURCE_LIMIT_EXCEEDED` → 413. Qualquer outro código `[X]` no prefix cai no mapeamento default 422 (conservador; HTTP 500 só no ramo 3).
- [ ] **Ramo 3 (fallback)** captura qualquer coisa que não seja `ZodError` nem `Error` com prefix — inclui `TypeError` de engine malformado pré-parse, etc. Status 500, `code: "UNKNOWN"`.
- [ ] Usar no endpoint `/api/calc/[...segments]/route.ts` envolvendo `getCompiled() + run()` num `try/catch` que delega para `handleCalcError(err)` no catch.

#### Versão final (Fase A2 — após runtime 1.4 entregar `CalcError`)

Quando a classe `CalcError` formal existir, os ramos 1 e 2 **colapsam em um único `if (err instanceof CalcError)`** que lê `err.code` direto:

```ts
export function handleCalcError(err: unknown): NextResponse {
  if (err instanceof CalcError) {
    const status = statusFromCode(err.code)  // INVALID_ENGINE→422, RESOURCE_LIMIT_EXCEEDED→413, ...
    return NextResponse.json(
...
- [ ] Refatorar `handleCalcError` removendo os ramos 1 e 2; a classe `CalcError` passa a carregar o código estruturado (inclui `INVALID_ENGINE` como sucessor direto do `ZodError`) e o handler lê direto.
- [ ] O fallback `UNKNOWN` permanece como rede de segurança para erros inesperados fora do domínio runtime.
- [ ] **Breaking visível:** o shape do response do ramo 1 muda de `{ error: "Invalid engine config", validationErrors: [...] }` para `{ error: { code: "INVALID_ENGINE", message, context } }`. Documentar no Changelog do APP + atualizar bruno + consumidores. Registrar no item correspondente do PLAN Fase 2.
- [ ] Manter a função `statusFromCode(code)` como único ponto que mapeia `CalcErrorCode → HTTP status` — fácil auditar e estender.

**Por quê:** hoje qualquer erro do `execute()` vira string solta no JSON (ou quebra no shape antigo de `{ error, validationErrors }`). Cliente não consegue rotear (ex.: mostrar toast para `INVALID_INPUT`, modal bloqueante para `RESOURCE_LIMIT_EXCEEDED`). A versão ponte entrega a estrutura nova sem esperar a Fase 2 do runtime; a versão final é drop-in quando `CalcError` existir. Manter isso documentado no **corpo** evita que a refatoração Fase A2 vire arqueologia de Changelog.

**Arquivos afetados:** novo `lib/server/handleCalcError.ts`, `app/api/calc/[...segments]/route.ts`. Futuro: bruno (`bruno/calc/*.bru`) e schemas (`schemas/api.ts` / `schemas/errors.ts` — ver E.3) quando a versão final mudar o shape.

**Risco:**
- Fase A1: baixo. O ramo 1 preserva o contrato legado; ramos 2 e 3 adicionam estrutura nova sem quebrar o que já existe.
- Fase A2: médio. O shape do ramo 1 muda — todos os consumidores que esperam `validationErrors` precisam migrar. Fazer junto com E.3 (schema de erro).

**Como verificar:**
- Fase A1: bruno dispara engine inválido → 422 com shape legado; bruno dispara engine com ciclo (simulado via DB) → 422 com `error.code === "CYCLE_DETECTED"`; bruno dispara engine acima de `maxSteps` → 413.
- Fase A2: mesmos cenários retornam shape unificado `{ error: { code, message, context } }`; bruno e snapshots atualizados.
...
- [ ] A.3 — Versão final do handler `CalcError` (colapsa ramos ZodError + prefix `[CODE]` em `instanceof CalcError`). Ponte já entregue na Fase A1.
...
- [ ] E.1 — Revisar `EngineSchema` / api.ts
- [ ] E.3 — Padronizar shape de erro no contrato API (par com A.3 final + runtime 1.4)
- [ ] F.3 — `exportState` usa `compile()`
- [ ] G.3 — Validação forte no JSON import/export com `compile()` (par com F.3; UX)

**Critério de saída:** nenhum arquivo de `lib/runtime/` importa código de UI; builder e test runner usam a mesma API canônica; contratos de API atualizados; comportamento de input inválido alinhado entre API/builder/calculator; shape de erro unificado entre runtime e HTTP.
...
**Objetivo:** observabilidade e UX refinada — visibility para skip/outputs; operações de dados auxiliares.

- [ ] A.4 — Pré-aquecimento opcional do cache no boot (feature flag `WARMUP_ENABLED`)
...
- [ ] G.2 — Script de migração (se runtime 3.7/3.10 precisar de backfill)

**Critério de saída:** debug mode mostra trace completo incluindo skips; consumidor do `/api/calc` pode escolher output por ID via UI; cache pré-aquecido em boot remove a "primeira request cara" de engines ativos; dados existentes migrados quando necessário.
...
- **2026-04-20** — **Decisões de execução da Fase A1** (pré-código, após revisão com Barney):
  - **A.2 escopo da invalidação.** Cache é invalidado apenas nos endpoints que mutam o JSON do `engine`:
    - `PATCH /api/engines/:id` — invalida sempre que o PATCH succeede (barato; cobre mudança de `engine`, `name` e `project_id`).
    - `DELETE /api/engines/:id` — invalida.
    - `POST /api/engines/:id/activate` — **NÃO invalida.** Esse endpoint só muta `is_active`, não muda `engine`, e `/api/calc/:id` não filtra por `is_active`. Invalidar seria desperdício.
    - `POST /api/engines` (create) — não invalida (engine novo nunca esteve no cache).
  - **A.1 localização do cache.** Cache `Map<engineId, CompiledEngine>` e helper `getCompiled` vivem em `lib/server/compiledCache.ts` (compartilhado entre `/api/calc` e os endpoints de invalidação). Module-scope, `runtime = "nodejs"` requerido na rota consumidora.
  - **A.3 mínima.** `CalcError` formal é Fase 2 do runtime. Até lá, `handleCalcError(err)` em `lib/server/handleCalcError.ts` encadeia três ramos, nesta ordem:
    1. `if (err instanceof ZodError)` → **422** com shape `{ error: "Invalid engine config", validationErrors: [{ path, message }] }`. Preserva 1-pra-1 o response atual do `execute()` facade quando o engine falha na validação Zod — zero regressão para bruno/flows e consumidores externos que já tratam `validationErrors`. `compile()` valida o engine via Zod internamente e lança `ZodError` cru em caso de falha; este ramo captura antes do parse de prefix.
    2. parse do prefix `^\[([A-Z_]+)\]\s*(.*)$` em `err.message` → mapeia `CYCLE_DETECTED` → 422 e `RESOURCE_LIMIT_EXCEEDED` → 413, com body `{ error: { code, message } }`.
    3. fallback → **500** `{ error: { code: "UNKNOWN", message } }`.

    Quando Fase 2 do runtime aterrissar com `CalcError` formal, os ramos 1 e 2 colapsam num único `if (err instanceof CalcError)` que lê `err.code` direto (incluindo `INVALID_ENGINE` como sucessor do `ZodError`). O fallback `UNKNOWN` permanece como rede de segurança.
...
- **2026-04-20** — **Cobertura completa das fases A1–A4.** Validação cruzada revelou 4 itens documentados no corpo das Camadas A–G que não apareciam em nenhuma fase (órfãos): **A.4** (pré-aquecimento), **E.3** (shape de erro padronizado), **G.2** (script de migração 3.7/3.10), **G.3** (validação forte no import). Também o item **A.3** tinha trabalho dividido entre Fase A1 (ponte) e Fase A2 (refactor final) mas só a ponte estava listada. Ajustes:
  - **Fase A2** passa de 6 para 9 itens: adicionados A.3 (completa), E.3, G.3.
  - **Fase A3** passa de 5 para 7 itens: adicionados A.4, G.2.
  - Contadores por fase no PROGRESS Parte 2 atualizados.
  - Motivação: prompts de execução (`@todo/RUNTIME_PROMPT_FASE_*.md`) derivam dessas listas; itens órfãos não iam aparecer no roteiro de nenhuma fase, vazariam para "depois" sem owner.

- **2026-04-20** — **Consolidação corpo-vs-Changelog.** Filosofia do projeto reafirmada: **corpo de cada doc deve ser sempre a solução final**, Changelog é apenas histórico. Decisões que viviam só no Changelog foram migradas para o corpo dos itens correspondentes:
  - **A.1 — caminho canônico `lib/server/compiledCache.ts` explícito no corpo**, com trecho de código mostrando `getCompiled` + `invalidateCompiled`. Justificativa de "lib/server/ vs lib/runtime/" adicionada ao "Por quê" (preserva fronteira da lib para extração futura).
  - **A.2 — tabela com os 4 endpoints de `/api/engines`** e política de invalidação (`PATCH` ✅, `DELETE` ✅, `activate` ❌, `create` ❌) agora vive no corpo como contrato explícito — qualquer endpoint futuro que mute o JSON do engine precisa ser adicionado à tabela. Passo de implementação `invalidateCompiled(engineId)` **após** a persistência documentado.
  - **A.3 — subseções "Versão ponte (Fase A1)" e "Versão final (Fase A2)"** com código completo dos dois handlers. A ponte explicita: (1) preservação 1-pra-1 do shape legado `{ error, validationErrors }` no ramo `ZodError`, (2) parse do prefix `^\[([A-Z_]+)\]\s*(.*)$` para `CYCLE_DETECTED`/`RESOURCE_LIMIT_EXCEEDED`, (3) fallback `UNKNOWN`/500. A versão final mostra o `if (err instanceof CalcError)` unificado + função `statusFromCode` como único ponto de mapeamento, e o breaking do shape do ramo 1 (que precisa coordenar com E.3 e bruno).
  - Motivação: LLM ou humano entrando na Fase A1 agora tem código colável + racional sem precisar reconstituir decisões lendo Changelog antigo.
