...
- [ ] **3.3** Ordenação topológica em `compile()` (topsort + tie-break pela ordem do array)
...
**Critério de saída:** cenário médio < 1ms p99, grande < 5ms p99; smoke test via bruno (`bruno/calc/*` e `bruno/flows/calc/*`) passando; app Next rodando normalmente.
...
- **Ordem de execução cronológica da Fase 1:** 1.1 → 3.11 → **2.7 (baseline obrigatório)** → 2.2 → 1.2 (+2.6) → 3.2/3.3 → 1.5. Ver bloco "Ordem de execução recomendada" em `RUNTIME_REFACTOR_PLAN.md` → Fase 1. Preencher "Histórico de medições" acima com o baseline **antes** de aplicar qualquer otimização (2.2, 1.2).
...
> **Nota (2026-04-20):** D.1 e D.2 marcados como concluídos (no-op após auditoria — ver `RUNTIME_REFACTOR_APP.md`). Camada A ganhou novo item A.6 (`onInvalidInput`), total passou de 25 → 26.

### Progresso por camada

| Camada | Escopo | Total | Feitos | % |
|---|---|---|---|---|
| A — API server-side | `/api/calc/...` | 6 | 0 | 0% |
| B — Cliente builder | `EngineBuilder`, `StepCard`, `TestPanel` | 5 | 0 | 0% |
| C — Cliente calc | `Calculator` | 2 | 0 | 0% |
| D — Tipos & shared state | `lib/types.ts`, hooks | 4 | 2 | 50% |
| E — Schemas & contratos API | `schemas/*` | 3 | 0 | 0% |
...
|---|---|---|---|---|---|
| A1 — Pré-produção | 5 | 2 | 40% | 🟡 Em andamento | Fase 1 Runtime |
| A2 — Pré-extração lib | 9 | 0 | 0% | 🔴 Não iniciada | Fase 2 Runtime |
| A3 — Otimização guiada | 7 | 0 | 0% | 🔴 Não iniciada | Fase 3 Runtime |
...
## Camada A — API server-side [0/6]

Endpoint principal: `app/api/calc/[...segments]/route.ts`.

- [ ] **A.1** Migrar handler para `compile()+run()` com cache `Map<engineId, CompiledEngine>` + `runtime = "nodejs"` *(depende de runtime 1.2)*
...
- [ ] **A.6** Política de `onInvalidInput` no endpoint + consumidores (builder, calculator) *(depende de runtime 3.1)*
...
## Camada D — Tipos & shared state [2/4]

Afeta `lib/types.ts`, `hooks/useEngineState.ts`, `lib/runtime/types.ts`.

- [x] **D.1** ~~Limpar `lib/types.ts`~~ — **no-op** (revisão 2026-04-20: `lib/types.ts` já é a casa canônica dos tipos UI)
- [x] **D.2** ~~Remover tipos UI de `lib/runtime/types.ts`~~ — **coberto por runtime 1.1** (nenhum trabalho adicional no app)
...
Afeta `schemas/api.ts`, `schemas/endpoints.ts`.
...
### Fase A1 — Pré-produção [2/5]

Acompanha Fase 1 do Runtime. Foco: deixar o app compilando e rodando sobre o novo compile/run sem regressão.

- [x] D.1 ~~Limpar `lib/types.ts`~~ (no-op — ver revisão 2026-04-20)
- [x] D.2 ~~Remover UI types de `lib/runtime/types.ts`~~ (coberto por runtime 1.1)
- [ ] A.1 compile+run + cache na API (com `runtime = "nodejs"`)
...
### Fase A2 — Pré-extração lib [0/9]

Acompanha Fase 2 do Runtime. Foco: eliminar qualquer uso que impediria a extração.

- [ ] A.3 Versão final do handler `CalcError` (colapsa ponte da Fase A1 em `instanceof CalcError`)
...
- [ ] E.3 Padronizar shape de erro no contrato API (par com A.3 final)
- [ ] F.3 `exportState` usa `compile()`
- [ ] G.3 Validação forte no JSON import (UX; par com F.3)

**Critério de saída:** nenhum arquivo de `lib/runtime/` é importado para fins de UI; builder/test runner usam mesma API canônica; contratos de API atualizados; comportamento de input inválido alinhado entre API/builder/calculator; shape de erro unificado entre runtime e HTTP.

---

### Fase A3 — Otimização guiada [0/7]

Acompanha Fase 3 do Runtime. Foco: observabilidade + UX refinada + operações de dados.

- [ ] A.4 Pré-aquecimento opcional do cache no boot (`WARMUP_ENABLED`)
...
**Critério de saída:** debug mode mostra trace completo incluindo skips; consumidor do `/api/calc` pode escolher output por ID; cache pré-aquecido em boot; dados existentes migrados quando necessário.
