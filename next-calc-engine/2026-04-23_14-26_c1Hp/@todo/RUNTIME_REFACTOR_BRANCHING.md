...
- **`stores/engineStore.ts` + `hooks/useEngineState.ts` herdam automaticamente** via import direto: ambos importam `Step`/`EngineState` de `@/lib/runtime`, logo `AppState.engine` e `EngineRecord.engine` ganham `when?` sem patch manual. O item `APP.D.4` exige **smoke check de round-trip**: editar `when` no builder, salvar no store, recarregar, confirmar que o valor persiste.
