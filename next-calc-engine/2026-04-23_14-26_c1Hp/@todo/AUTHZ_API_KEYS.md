...
Sem policy de UPDATE, um editor autenticado não consegue via PostgREST alterar `role`, `key_hash` ou `deleted_at` de uma key existente — elimina o vetor de escalação de privilégio e de hijack silencioso de key. O soft-delete (setar `deleted_at`) é feito pelo handler via `DELETE` lógico (`PATCH` de `deleted_at` nunca exposto) ou, se necessário, via admin client no handler de revogação.

**Nota:** o handler de revogação (`DELETE /api/tenants/[id]/api-keys/[keyId]`) deve usar `createAdminClient()` para o soft-delete (`UPDATE api_keys SET deleted_at = now() WHERE id = ...`), já que a policy de UPDATE está revogada para o client autenticado. Criação e listagem continuam via client autenticado com RLS.
