...
- **2026-04-20** — **Decisões de execução da Fase A1** (pré-código, após revisão com Barney):
  ...
  - **A.3 mínima.** `CalcError` formal é Fase 2 do runtime. Até lá, `handleCalcError(err)` em `lib/server/handleCalcError.ts` faz parse do prefix `[CODE]` no início de `err.message` e mapeia:
    - `CYCLE_DETECTED` → 422
    - `RESOURCE_LIMIT_EXCEEDED` → 413
    - erro sem prefix conhecido → 500 genérico
    Response body padronizado: `{ error: { code, message } }`. Quando Fase 2 aterrissar com `CalcError` formal, o helper ganha `if (err instanceof CalcError) return ...` acima do parse, sem remover o fallback (retrocompat com erros legados).
