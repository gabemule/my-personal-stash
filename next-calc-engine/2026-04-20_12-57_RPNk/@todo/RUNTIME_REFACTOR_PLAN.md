...
`CompiledEngine` é um objeto **em memória** (não um artefato em disco) carregando tudo que o `run()` precisa **sem ter que re-parsear nem re-construir nada**. Shape proposto (campos `readonly`, imutável por construção):
...
interface CompiledEngine {
  ...
  // ordem de execução + análise estática
  readonly executionOrder: readonly string[]
  readonly outputStepIds: readonly string[]

  // metadados (debug/introspecção)
  readonly meta: {
    readonly indexedTables: ReadonlySet<string>  // tabelas com busca binária (ver 2.4)
    readonly warnings: readonly string[]          // unreachable steps, migrações implícitas, etc.
  }
}
```

Sub-estruturas importantes:
- `CompiledStep`: `id`, `name`, `enabled`, `kind` (normalizado), `clamp` (normalizado), `when` (compilado, ver 4.1), `expression: readonly CompiledToken[]`, `dependencies: ReadonlySet<string>`.
- `CompiledToken` / `CompiledConditionSide`: `{ type: "number", value, decimal: Decimal }` — literais pré-parseados.
- `CompiledTable`: `columns`, `rows` com valores pré-parseados, `rowIndex: RowIndex | null` (busca binária quando aplicável).

**Chave:** o hot-path do `run()` não faz mais `Zod.parse`, nem `Decimal.clone`, nem `Object.fromEntries`, nem `new Decimal("literal")`. Tudo isso já está pronto.

#### 1.2.b Workflow interno do `compile()`

```
compile(engine) →
  [A] EngineSchema.safeParse        (Zod; defaults aplicados; INVALID_ENGINE se falhar)
  [B] Construir tablesMap, variablesMap, stepsMap
  [C] Obter D via createDecimalFactory com cache (ver 2.2)
  [D] Pré-computar Decimais de todos os literais (tokens, condições, defaults, min/max, células)  (ver 2.3)
  [E] Análise estática:
        - grafo de dependências (stepRef em expressions + when)
        - detecção de ciclo  (CYCLE_DETECTED)
        - forward-ref check  (FORWARD_REF)
        - warnings (unreachable steps)
  [F] Indexação de tabelas de faixas numéricas (ver 2.4)
  [G] Guards de recursos (ver 1.5)  (RESOURCE_LIMIT_EXCEEDED)
...
- [ ] Registrar baseline (antes das otimizações) e target (depois) em `RUNTIME_REFACTOR_PROGRESS.md`.
...
3. **2.7** — **BASELINE OBRIGATÓRIO.** Instalar tinybench, escrever os 3 cenários, rodar `yarn bench` contra a lib **pré-refactor**, registrar números brutos em `RUNTIME_REFACTOR_PROGRESS.md` → "Histórico de medições". Sem baseline, otimização vira chute.
