...
- **`lib/types.ts` + `stores/engineStore.ts` herdam automaticamente** via re-export: `lib/types.ts` re-exporta `Step` direto de `@/lib/runtime`, logo `AppState.engine` e `EngineRecord.engine` ganham `when?` sem patch manual. O item `APP.D.4` exige **smoke check de round-trip**: editar `when` no builder, salvar no store, recarregar, confirmar que o valor persiste.
...
- [ ] **B.5** `outputs: Record<stepId, string | null>` em `ExecuteResult`
