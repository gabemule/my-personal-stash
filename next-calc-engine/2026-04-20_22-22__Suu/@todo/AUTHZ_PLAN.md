...
**Regra de ouro:** execução estritamente sequencial. Fase 3 depende do union `auth.via: "cookie" | "bearer"` introduzido em Fase 2 (as rotas de members/invites/transfer-ownership/billing-owner fazem `if (auth.via === "bearer") return 403 COOKIE_REQUIRED`). Fase 4 reusa a UI de Fase 3 (modal convidar, lista de pendentes).
...
Diferente do fluxo inicial considerado, **a criação de tenants NÃO é self-service**. Apenas super admin cria, via `POST /api/tenants` (cookie + flag `is_super_admin`) → chama a RPC `create_tenant(_name, _owner_id, _billing_id)`. Motivos:
