...
| `app/admin/tenants/new/page.tsx` | Criar | Form super admin only: `name` apenas; `owner_id`/`billing_id` defaultam para o criador. |
...
| PATCH | `/api/tenants/[id]` | primary owner ou super admin | `rename_tenant` | `INSUFFICIENT_ROLE` (403) |
...
| `INSUFFICIENT_ROLE`, `ONLY_*_CAN_*`, `NOT_AUTHORIZED`, `ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_RENAME_TENANT` | 403 |
| demais | 500 |
...
- `POST /api/session/current-tenant`:
  - Valida `tenantId ∈ session.tenants` (ou super admin); usa `setTenantCookie(response, tenantId)` para setar `next-calc-current-tenant`.

**Helper `setTenantCookie(response, tenantId)`:** extrai a lógica de cookie em um único lugar (`lib/cookies.ts` ou similar) com flags consistentes: `httpOnly: true`, `sameSite: "lax"`, `path: "/"`, `secure: process.env.NODE_ENV === "production"`. Reutilizado em `POST /api/session/current-tenant` e em `POST /api/invites/[token]/accept` (Fase 4) — ambos os handlers setam o mesmo cookie e devem usar a mesma configuração.
...
### `settings/page.tsx`
- Input nome (**primary owner / super admin** — secondary owners não renomeiam) → PATCH.
...
- `app/admin/tenants/new/page.tsx` — super admin only (layout já garante o redirect); form com `name` apenas. `owner_id` e `billing_id` defaultam para o super admin criador via RPC; transferência para owner definitivo via `transfer_primary_ownership` + `set_billing_owner` depois.
...
- **CSRF:** rotas cookie-based não têm CSRF token explícito. `SameSite: "lax"` mitiga: cross-origin `fetch` com método `DELETE`/`POST` não carrega cookies em contexto cross-site. `DELETE /api/tenants/[id]` (soft-delete de tenant) é o caso de "operação irreversível cookie-based" — coberto pela mesma proteção `SameSite: lax` (fetch cross-site não carrega cookie de sessão). Reavaliar apenas se surgir cenário de subdomain ou relaxamento de `SameSite`.
