...
- `projects`: adicionar `tenant_id uuid NOT NULL references tenants(id) ON DELETE RESTRICT` (nullable durante migration, depois `set not null`), `deleted_at timestamptz`.
- `engines`: adicionar `deleted_at timestamptz`. Herda tenant via `project_id`.
- **`engines.project_id NOT NULL`**: após o backfill de engines órfãs na migration, aplicar `ALTER TABLE engines ALTER COLUMN project_id SET NOT NULL`. Todo engine precisa de um project para que a chain `engine → project → tenant` da RLS funcione corretamente — engine sem `project_id` ficaria inacessível para todos via RLS (sem project = sem tenant = sem role). A FK mantém `ON DELETE SET NULL` do schema original **até** o `SET NOT NULL` ser aplicado; depois disso alterar explicitamente para `ON DELETE RESTRICT` via migration step 8 (DROP + ADD CONSTRAINT) — `ON DELETE SET NULL` + `NOT NULL` resultaria em erro críptico de constraint ao invés de um RESTRICT limpo.
...
`true` se caller é super admin OU tem row em `tenant_members` com role ≥ `_min_role`. Para non-super-admin, a checagem inclui `JOIN tenants WHERE deleted_at IS NULL` — membros de tenants soft-deleted não são autorizados. Super admin bypassa o filtro de `deleted_at` (precisa operar em tenants soft-deleted para fins de restauração). Hierarquia: `owner > manager > editor > reader`. `stable`, `security definer`, `set search_path = ''`.
...
### `create_tenant(_caller_id, _caller_is_super_admin, _name, _owner_id default null, _billing_id default null) returns uuid`
...
- **`_owner_id` e `_billing_id` são opcionais** — quando omitidos, default = `_caller_id`. O super admin criador vira primary owner + billing temporariamente; a transferência para o owner definitivo ocorre depois via `transfer_primary_ownership` + `set_billing_owner`. O handler de criar tenant **não exige** esses campos — form de UI só precisa de `name`.
- **Guard defensivo:** mesmo que `_caller_is_super_admin = true`, a RPC verifica `is_super_admin(_caller_id)` diretamente — isso protege contra o cenário em que o handler passou um parâmetro stale (ex: flag de super admin recém revogada mas JWT ainda não expirado). Se a verificação interna falhar, erro `ONLY_SUPER_ADMIN_CAN_CREATE_TENANT`.
...
- Super admin ou **primary owner** (`tenants.owner_id`) exclusivamente. Secondary owners não têm esta permissão.
- Erros: `NAME_REQUIRED`, `TENANT_NOT_FOUND`, `ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_RENAME_TENANT`, `TENANT_NAME_TAKEN` (captura `unique_violation` 23505 do unique index `tenants_name_unique`).
...
- **Locking:** a RPC adquire lock exclusivo nas rows de `tenant_members` do tenant antes de validar o role do `_new_owner_id` — elimina a race condition com uma remoção ou rebaixamento concorrente desse membro.
- Erros: `TENANT_NOT_FOUND`, `NOT_AUTHORIZED`, `NEW_OWNER_NOT_OWNER_ROLE`, `ALREADY_PRIMARY_OWNER` (quando `_new_owner_id = tenants.owner_id` — no-op explicitamente rejeitado).

### `set_billing_owner(_caller_id, _caller_is_super_admin, _tenant_id, _user_id) returns void`

- Super admin ou primary owner atual.
- `_user_id` precisa ter `role='owner'`.
- **Locking:** idem `transfer_primary_ownership` — lock exclusivo em `tenant_members` antes de validar o role do `_user_id`.
...
- Apenas super admin.
- **Guard defensivo:** mesmo que `_caller_is_super_admin = true`, a RPC verifica `is_super_admin(_caller_id)` diretamente antes de qualquer mutação — idem `create_tenant`, pelo mesmo motivo (parâmetro potencialmente stale).
...
- Erros: `ONLY_SUPER_ADMIN_CAN_FORCE_REMOVE`, `NEW_PRIMARY_SAME_AS_TARGET`, `NEW_BILLING_SAME_AS_TARGET`, `TENANT_NOT_FOUND`, `NEW_BILLING_ID_REQUIRED`.
...
6. Associar engines órfãs ao project correto (ou deletar): `update engines set project_id = '<project-id>' where project_id is null;` — rodar uma vez por engine pendente do Step 0.
7. `alter table engines alter column project_id set not null;` — após confirmar que não existe nenhuma engine com `project_id is null`. Todo engine precisa de um project; engines sem project ficam inacessíveis via RLS.
8. `alter table engines drop constraint engines_project_id_fkey; alter table engines add constraint engines_project_id_fkey foreign key (project_id) references projects(id) on delete restrict;` — substitui o `ON DELETE SET NULL` original pelo `ON DELETE RESTRICT` correto agora que `project_id` é `NOT NULL`.
