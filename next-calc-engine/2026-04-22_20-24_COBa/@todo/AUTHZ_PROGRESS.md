...
- [ ] Criar `db/authz_rpcs.sql` com RPCs atômicas (todas recebem `_caller_id uuid` + `_caller_is_super_admin boolean` explícitos): `create_tenant`, `rename_tenant`, `transfer_primary_ownership`, `set_billing_owner`, `delete_tenant`, `force_remove_primary_owner`, `change_member_role`, `remove_member`, `accept_invite`, `create_invite`, `revoke_invite`
- [ ] Reescrever `db/rls.sql` com policies por tenant (incluindo cascata de soft-delete em projects/engines joining `tenants.deleted_at` + revogar insert/update/delete em `tenant_members`, `tenants` e `tenant_invites` — todas as mutações via RPCs)
...
- [ ] Smoke test RPCs protegidas: `supabase.rpc('change_member_role', ...)` / `rpc('create_tenant', ...)` / `rpc('create_invite', ...)` direto retorna erro de permissão (revogados de anon/authenticated — só admin client invoca)
- [ ] Smoke test mutações bloqueadas nas tabelas: `insert/update/delete` direto em `tenants` e `tenant_invites` via client autenticado retorna erro de RLS (toda mutação passa por RPC)
...
### Settings > API Keys
- [ ] Rota `app/(authed)/tenants/[id]/settings/api-keys/page.tsx` — listar keys com badge de role (editor+), modal "Nova Key" com select `role` (`reader`/`editor`, default `reader`) e raw mostrado 1x, botão revogar

### Settings > Tenant
- [ ] Rota `app/(authed)/tenants/[id]/settings/page.tsx` (editar nome, ver info)
...
- **2026-04-22 (fixes críticos Part 2 — admin client + tenant_invites RPCs + accept_invite guard + (authed) settings):**
  - **Critical fix — RPCs invocadas via `createAdminClient().rpc(...)`.** Como `revoke execute on ... from anon, authenticated` está aplicado em todas as RPCs (e nos helpers), chamar via `auth.supabase.rpc(...)` (client do user autenticado) retornaria `permission denied`. Todos os handlers de gestão (`app/api/tenants/*`, invites, members, transfer, billing) passam a instanciar `const admin = createAdminClient()` e delegar `admin.rpc("...", {...})`. A identidade do caller continua vindo do `requireUserAuth` e é passada explicitamente via `_caller_id` / `_caller_is_super_admin` — service role não vira back-door, só transporte.
  - **Critical fix — `tenant_invites` writes concentrados em RPCs.** `db/rls.sql` passa a revogar `insert/update/delete on tenant_invites from anon, authenticated` (policy `"managers manage invites"` removida). Novas RPCs `security definer` em `db/authz_rpcs.sql`:
    - `create_invite(_caller_id, _caller_is_super_admin, _tenant_id, _email, _role, _token, _force)` → `table(id, token)` — gate `has_tenant_role('manager')`, `role='owner'` exige primary/super, mapeia `23505` → `PENDING_INVITE_EXISTS`, honra `_force` para reenviar.
    - `revoke_invite(_caller_id, _caller_is_super_admin, _tenant_id, _invite_id)` — gate `has_tenant_role('manager')`, seta `revoked_at = now()`.
    - `POST /api/tenants/[id]/invites` e `DELETE /api/tenants/[id]/invites/[inviteId]` reescritos para delegar a essas RPCs; código de erro `INVALID_ROLE` (400), `TENANT_NOT_FOUND` (404), `INSUFFICIENT_ROLE` / `ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_INVITE_OWNER` (403), `PENDING_INVITE_EXISTS` (409).
  - **Critical fix — `accept_invite` guarda primary/billing owner.** Quando `_caller_id` é `tenants.owner_id` ou `tenants.billing_id` e a membership resultante teria `role <> 'owner'`, a RPC aborta com `CANNOT_MODIFY_PRIMARY_OWNER` / `CANNOT_MODIFY_BILLING_OWNER`. Fecha o buraco de aceitar convite de `manager` enquanto já é primary owner e acabar rebaixado silenciosamente.
  - **Critical fix — `rename_tenant` RPC.** `PATCH /api/tenants/[id]` passa a delegar para `rename_tenant(_caller_id, _caller_is_super_admin, _tenant_id, _new_name)` (gate `has_tenant_role('owner')`). Combinado com o revoke de writes em `tenants`, toda mutação de tenants (create/rename/transfer-primary/set-billing/delete) é agora 100% via RPC. Tabela `tenants` não aceita mais `update` direto de client autenticado.
  - **Critical fix — Settings pages sob `(authed)` route group.** Paths corrigidos em `AUTHZ_RBAC_UI.md` e `AUTHZ_INVITES.md`: `app/tenants/[id]/settings/...` → `app/(authed)/tenants/[id]/settings/...` (members, api-keys, page root). Fora do grupo, as pages não herdariam o layout autenticado (`getSessionContext`, `TenantSelector`, redirects de `/no-tenant` / super-admin) e acabariam renderizando sem hidratar `tenantStore` — quebra de UX e buraco de auth silencioso.
  - Contagens reajustadas: Fase 1 29 → 32 (3 novas RPCs: `rename_tenant`, `create_invite`, `revoke_invite`) · Fase 3 inalterada (renomes de path só) · **total 74 → 77**.
