...
- **Mutações sensíveis concentradas em RPCs `security definer`:** além de `tenant_members` (criação/mudança de role/remoção), **também** `tenants` (update de nome / soft-delete) e `tenant_invites` (insert/update/delete) são bloqueados diretamente em RLS (`revoke insert, update, delete`) e expostos via RPC. Isso garante que invariantes como `ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_INVITE_OWNER`, `CANNOT_MODIFY_PRIMARY_OWNER` e a imutabilidade de `tenants.owner_id/billing_id/deleted_at` (exceto via RPCs dedicadas) sejam enforçadas em **um único lugar**, sem possibilidade de bypass via PostgREST / `supabase.from(...)`.
- **RPCs são chamadas via service-role (admin client), não via sessão do user.** Como `revoke execute ... from authenticated` é aplicado em todas as RPCs (§11), o handler HTTP usa `createAdminClient().rpc(...)` passando `_caller_id` + `_caller_is_super_admin` extraídos de `requireAuth(...)`. O admin client existe apenas no server; o caller continua sendo o user autenticado (ou super admin) — só o canal de invocação é service-role.
...
-- ---------- TENANTS ----------
-- SELECT é aberto a membros + super admin; TODAS as mutações (insert/update/
-- delete) ficam bloqueadas direto e só passam pelas RPCs `create_tenant`,
-- `rename_tenant`, `transfer_primary_ownership`, `set_billing_owner`,
-- `delete_tenant` e `force_remove_primary_owner` (§11). Isso fecha o gap de
-- um OWNER secundário conseguir sobrescrever `owner_id`/`billing_id`/`deleted_at`
-- via `supabase.from('tenants').update(...)` burlando as invariantes.
...
-- INSERT/UPDATE/DELETE: bloqueados. Criação e revogação SÓ via RPCs
-- `create_invite` e `revoke_invite` (§11). Isso fecha o bypass de um
-- MANAGER inserir direto com `role='owner'` via PostgREST — o gate
-- ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_INVITE_OWNER é enforçado num
-- único lugar (a RPC).
...
revoke insert, update, delete on tenant_invites from anon, authenticated;
-- (SELECT continua permitido pela policy acima.)
...
begin
  ...
  -- Guard: o caller é o primary ou billing owner atual, e o convite é pra
  -- role < owner? O upsert abaixo rebaixaria o primary/billing em violação
  -- das invariantes (b)/(c) do §Decisões-chave. Bloqueia.
  -- (Convite pra role='owner' é idempotente pro primary/billing — upsert
  -- mantém role='owner'.)
  select owner_id, billing_id into v_owner_id, v_billing_id
  from tenants where id = v_tenant_id;

  if v_role <> 'owner' and _caller_id = v_owner_id then
    raise exception 'CANNOT_MODIFY_PRIMARY_OWNER';
  end if;
  if v_role <> 'owner' and _caller_id = v_billing_id then
    raise exception 'CANNOT_MODIFY_BILLING_OWNER';
  end if;
...
-- Rename do tenant (única mutação "direta" sobre tenants sem reflexo em
-- ownership/soft-delete). Owner (qualquer — primary ou secundário) ou super
-- admin. Centralizada aqui porque o RLS revoga UPDATE/INSERT/DELETE direto
-- sobre `tenants` (§4).
...
begin
  ...
  select exists (
    select 1 from tenants where id = _tenant_id and deleted_at is null
  ) into v_exists;
  ...
  if not _caller_is_super_admin
     and not exists (
       select 1 from tenant_members
        where tenant_id = _tenant_id and user_id = _caller_id and role = 'owner'
     ) then
    raise exception 'INSUFFICIENT_ROLE';
  end if;

  update tenants set name = _new_name where id = _tenant_id;
end;
$$;

-- Criação de convite. Centraliza o gate
-- ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_INVITE_OWNER (antes checado no
-- handler, agora inescapável) + unicidade pending + branch `force=true`
-- que revoga o pendente anterior antes de inserir o novo.
-- Retorna (id, token) do invite recém-criado.
create or replace function public.create_invite(
  ...
  _email text,
  _role text,
  _token text,           -- gerado no handler (crypto.randomBytes → base64url)
...
declare
  ...
  v_caller_role text;
  v_existing_id uuid;
  v_email text := lower(_email);
begin
  if _role not in ('owner','manager','editor','reader') then
    raise exception 'INVALID_ROLE';
  end if;
  ...
  -- Caller precisa ser manager+ (ou super admin).
  if not _caller_is_super_admin and v_caller_role not in ('owner','manager') then
    raise exception 'INSUFFICIENT_ROLE';
  end if;

  -- Gate pra role='owner': apenas primary owner ou super admin.
  if _role = 'owner'
     and not _caller_is_super_admin
     and v_owner_id <> _caller_id then
    raise exception 'ONLY_PRIMARY_OWNER_OR_SUPER_ADMIN_CAN_INVITE_OWNER';
  end if;

  -- Checa pending existente
  ...
  if v_existing_id is not null and not _force then
    raise exception 'PENDING_INVITE_EXISTS';
  end if;

  if v_existing_id is not null and _force then
    update tenant_invites set revoked_at = now() where id = v_existing_id;
  end if;

  insert into tenant_invites (tenant_id, email, role, token, invited_by)
  values (_tenant_id, v_email, _role, _token, _caller_id)
  returning tenant_invites.id, tenant_invites.token into id, token;

  return next;
end;
$$;

-- Revogação de convite pendente. Manager+ ou super admin. Idempotente: se
-- já estiver revogado/aceito, retorna sem erro (o handler chama só via
-- UI e o visual já reflete o estado).
...
begin
  select tenant_id into v_invite_tenant
  from tenant_invites where id = _invite_id;

  if v_invite_tenant is null or v_invite_tenant <> _tenant_id then
...
revoke execute on function public.rename_tenant(uuid, boolean, uuid, text)                         from anon, authenticated;
revoke execute on function public.create_invite(uuid, boolean, uuid, text, text, text, boolean)    from anon, authenticated;
revoke execute on function public.revoke_invite(uuid, boolean, uuid, uuid)                         from anon, authenticated;
```

> **Uso nas rotas:** todas rodam via **service-role** (`createAdminClient().rpc(...)`)
> porque `execute` foi revogado de `anon`/`authenticated` — sem o admin client
> o PostgREST rejeita a chamada. O handler extrai `_caller_id` +
> `_caller_is_super_admin` do objeto `Auth` (§8-9, kind === "user"), valida
> e **passa explicitamente** pra RPC. O canal é service-role; o caller lógico
> continua sendo o user autenticado. Exemplo canônico:
> ```ts
...
> const { error } = await admin.rpc("transfer_primary_ownership", {
...
> ```
>
> - `POST /api/tenants` → `admin.rpc('create_tenant', { _caller_id, _caller_is_super_admin, _name, _owner_id, _billing_id })`.
> - `PATCH /api/tenants/[id]` → `admin.rpc('rename_tenant', { _caller_id, _caller_is_super_admin, _tenant_id, _new_name })`.
> - `POST /api/tenants/[id]/transfer-ownership` → `admin.rpc('transfer_primary_ownership', { _caller_id, _caller_is_super_admin, _tenant_id, _new_owner_id })`.
> - `POST /api/tenants/[id]/billing-owner` → `admin.rpc('set_billing_owner', { _caller_id, _caller_is_super_admin, _tenant_id, _user_id })`.
> - `DELETE /api/tenants/[id]` → `admin.rpc('delete_tenant', { _caller_id, _caller_is_super_admin, _tenant_id })`.
> - `PATCH /api/tenants/[id]/members` → `admin.rpc('change_member_role', { _caller_id, _caller_is_super_admin, _tenant_id, _user_id, _new_role })`.
> - `DELETE /api/tenants/[id]/members/[userId]` → `admin.rpc('remove_member', ...)` (target regular) ou `admin.rpc('force_remove_primary_owner', { _caller_id, _caller_is_super_admin, _tenant_id, _target_user_id, _new_primary_id, _new_billing_id })` quando target = `owner_id` (super admin).
> - `POST /api/tenants/[id]/invites` → `admin.rpc('create_invite', { _caller_id, _caller_is_super_admin, _tenant_id, _email, _role, _token, _force })`.
> - `DELETE /api/tenants/[id]/invites/[inviteId]` → `admin.rpc('revoke_invite', { _caller_id, _caller_is_super_admin, _tenant_id, _invite_id })`.
> - `POST /api/invites/[token]/accept` → `admin.rpc('accept_invite', { _caller_id, _caller_email, _invite_id })`.
