...
Os schemas de contrato de API (`lib/schemas/api.ts`, `lib/schemas/endpoints.ts`) importam `EngineSchema` direto da lib; stores (`stores/engineStore.ts`, `stores/requestStore.ts`) e `lib/exportState.ts` completam o acoplamento.
...
**Risco:** médio. O cache precisa ser invalidado corretamente (ver A.2) — cache stale servido após save do engine é bug crítico silencioso.
...
**Arquivos afetados:** `app/api/calc/[...segments]/route.ts`, schemas em `lib/schemas/`.
...
> Complementa: **runtime 1.1** (que remove UI types de `lib/runtime/types.ts`).

- [ ] Após `lib/runtime/types.ts` deixar de exportar `UIState`, `AppState`, `TestResult`, `EditingNumber`, revisar `lib/types.ts`.
- [ ] Se hoje re-exporta via `export * from "lib/runtime/types"`, garantir que os tipos UI continuam disponíveis localmente (hoje eles também estão em `lib/types.ts`).
- [ ] Consolidar os tipos UI em um único lugar (provavelmente `lib/types.ts` ou `lib/ui/types.ts`); remover duplicação.
- [ ] Atualizar consumidores: `hooks/useEngineState.ts`, stores, componentes.

**Por quê:** evitar duplicação é o motivo de remover os UI types da lib. Se o app também duplica, ganho zero.

**Arquivos afetados:** `lib/types.ts`, consumidores em `hooks/`, `stores/`, componentes.

**Risco:** baixo. TS acusa imports quebrados.

**Como verificar:** `tsc --noEmit` verde; `grep` por cada tipo confirma único ponto de declaração.

---

### D.2 Remover tipos UI de `lib/runtime/types.ts`

> Par direto do **runtime 1.1**. Listado aqui também porque requer trabalho no app (mover consumidores).

- [ ] Coordenado com o item 1.1 do PLAN.
- [ ] Critério: após concluído, `lib/runtime/types.ts` só tem tipos de domínio do runtime (Engine, Step, Variable, etc.).

**Por quê:** precondição para extração da lib como pacote independente.

**Arquivos afetados:** coordenado com runtime 1.1.

**Risco:** baixo (mesmo do runtime 1.1).

**Como verificar:** mesmo critério do runtime 1.1 + app builda sem regressão.
...
Escopo: `lib/schemas/api.ts`, `lib/schemas/endpoints.ts`.
...
**Arquivos afetados:** `lib/schemas/api.ts`, `lib/schemas/endpoints.ts`.
...
**Arquivos afetados:** `lib/schemas/endpoints.ts`.
...
**Arquivos afetados:** `lib/schemas/api.ts` (ou novo `lib/schemas/errors.ts`).
...
**Pré-requisito:** Fase 1 do PLAN em andamento (pelo menos itens 1.1, 1.2, 1.4 começados).

**Objetivo:** app compilando e rodando sobre o novo `compile/run` sem regressão funcional.

- [ ] D.1 — Limpar `lib/types.ts`
- [ ] D.2 — Remover UI types de `lib/runtime/types.ts` (par com runtime 1.1)
- [ ] A.1 — compile+run + cache na API
- [ ] A.2 — Invalidação do cache
- [ ] A.3 — Handler de `CalcError` no endpoint

**Critério de saída:** API `/api/calc/:id` responde no mesmo shape atual; cache validado manualmente (logs hit/miss); app Next rodando normalmente; nenhum UI type residual em `lib/runtime/`.
...
**Critério de saída:** nenhum arquivo de `lib/runtime/` importa código de UI; builder e test runner usam a mesma API canônica; contratos de API atualizados.
