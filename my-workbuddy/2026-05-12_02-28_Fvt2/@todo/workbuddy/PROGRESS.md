...
**Status:** 50/56 items · Phase: Feature Add File / Ingest / Lint

## Current Focus
[Feature: separar Add File, Ingest e Lint na sidebar]
Next step: Criar /add-file page, refatorar /ingest, criar /lint, criar POST /api/lint
Blocker: none
...
- [x] Dashboard — server component com stats reais (spaces, wiki pages, raw files, last ingest, recent activity)
- [x] Chat — multi-turn (envia histórico completo para o LLM, botão "Clear")
- [x] Ingest — toasts de sucesso/erro substituem mensagens inline
- [x] api/chat — aceita `messages[]` para contexto multi-turn
- [x] Build ✅ — Dashboard agora é `ƒ` (Dynamic) com stats reais
...
- 2026-05-11: Dashboard convertido para server component puro — lê filesystem diretamente sem fetch para si mesmo.
- 2026-05-11: Chat multi-turn: frontend envia `messages[]` completo; backend usa histórico anterior como contexto, mas injeta wiki context apenas na última mensagem do usuário.
- 2026-05-11: Add File / Ingest / Lint separados na sidebar — cada um com responsabilidade clara: Add File = criar raw + auto-ingest; Ingest = processar raw files existentes (incluindo staged); Lint = revisar wiki pages via LLM.

### Feature: Add File / Ingest / Lint
- [ ] Sidebar: adicionar Add File (/add-file), Ingest (/ingest), Lint (/lint) como itens separados
- [ ] /add-file page — form manual + auto-ingest após salvar + commit
- [ ] /ingest page — lista raw files + staged files, ingest selected + commit
- [ ] /lint page — lista wiki pages, lint selected
- [ ] POST /api/lint route (streaming SSE)
- [ ] Build + pstash save
